...
class ReedICML2016(data.Dataset):
    def __init__(self, img_root, caption_root, classes_fllename,
                 word_embedding, max_word_length, img_transform=None, seg_img_transform=None):
        super(ReedICML2016, self).__init__()
        self.alphabet = "abcdefghijklmnopqrstuvwxyz0123456789-,;.!?:'\"/\\|_@#$%^&*~`+-=<>()[]{} "

        self.max_word_length = max_word_length
        self.img_transform = img_transform
        self.seg_img_transform = seg_img_transform
    ...
    def _get_word_vectors(self, desc, word_embedding):
        ...
        for i in range(desc.shape[1]):
            ...
            word_list = [word_embedding.get_word_vector(w) for w in words]
            
            
            word_vecs = torch.from_numpy(np.asarray(word_list))
